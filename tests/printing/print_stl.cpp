...
TEST_CASE("std::ostream<<(std::map)") {
  ...
  SECTION("Non-Empty"){
    std::map<std::string, double> d{{"Element 1", 1.23}, {"Element 2", 2.34}};
    ss << d;
    std::stringstream corr;
...
TEST_CASE("std::ostream<<(std::multimap)") {
  ...
  SECTION("Non-Empty"){
    std::multimap<std::string, double> d{{"Element 1", 1.23}, {"Element 2", 2.34}};
    ss << d;
    std::stringstream corr;
    corr << "{(Element 1 : " << 1.23 << "), (Element 2 : " << 2.34 <<")}";
    REQUIRE(ss.str() == corr.str());
...
TEST_CASE("Nesting") {
  std::vector<std::map<int, double>> c {
      {{0, 1.1}, {1, 2.2}, {2, 3.3}},
      {{0, 4.4}, {1, 5.5}, {2, 6.6}}
  };
  std::stringstream ss;
