...
template<std::size_t tdepth, typename IteratorType>
auto reset_tuple_of_itr(IteratorType itr) {
    static_assert(tdepth != 0, "Can not reset first iterator");
    ...
    if constexpr(tdepth == tuple_length) {
        ...
        return reset_tuple_of_itr<tdepth + 1>(std::move(itr));
    }
}
...
auto increment_tuple_of_itr(ContainerType&& container, IteratorType itr) {
    ...
    return reset_tuple_of_itr<curr_idx + 1>(std::move(itr));
}
