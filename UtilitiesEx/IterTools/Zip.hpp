...
template <typename... ContainerTypes> class ZipImpl;

/// Struct which contains value==false if T is not a ZipImpl instance
template <typename T> struct IsZipImpl : std::false_type {};
...
public:
   ...
   *
   *  @param rhs The ZipImpl to take the state of.  After this call @p rhs is in
...
private:
  ...
  private:
    ...
    struct CallBegin {
      template <typename T> auto operator()(T container) {
    ...
    struct CallEnd {
      template <typename T> auto operator()(T container) const {
    ...
    struct Derefer {
      template <typename T> auto operator()(T itr) const { return *itr; }
    };
    ...
    struct AnyTrue {
      template <typename T> bool operator()(bool val, T element) const {
        return val || element;
      }
    };
  };
