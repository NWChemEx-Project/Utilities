...
private:
  ...
  template <typename T> struct AnyWrapper_ : public AnyBase_ {
  ...
  std::unique_ptr<AnyBase_> wrap_ptr(Args &&... args) {
    ...
    return std::move(std::make_unique<result_t>(std::forward<Args>(args)...));
  }
 ...
 */
template <typename T> T &AnyCast(Any &wrapped_value) {
 ...
 * convertible to type @p T.  Strong throw guarantee.
 */
template <typename T> const T &AnyCast(const Any &wrapped_value) {
 ...
 *
 */
template <typename T, typename... Args> Any MakeAny(Args &&... args) {
