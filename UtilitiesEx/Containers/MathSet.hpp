 ...
 */
template <typename element_type> class MathSet {
private:
...
public:
  ...
  template <typename input_element> auto insert(input_element &&ei) {
  ...
  bool operator<(const MathSet &rhs) const noexcept {
    if (size() >= rhs.size()) // Can't be subset if we don't have less elements
      return false;
    return operator<=(rhs);
  }
   ...
   * @throw None. No throw guarantee.
   */
  bool operator>=(const MathSet &rhs) const noexcept { return rhs <= (*this); }
